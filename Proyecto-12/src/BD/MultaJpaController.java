...
public class MultaJpaController implements Serializable {
    ...
    public void create(Multa multa) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            Coche vehiculo = multa.getVehiculo();
            if (vehiculo != null) {
                vehiculo = em.getReference(vehiculo.getClass(), vehiculo.getMatricula());
                multa.setVehiculo(vehiculo);
            }
            em.persist(multa);
            if (vehiculo != null) {
                vehiculo.getMultaList().add(multa);
                vehiculo = em.merge(vehiculo);
            }
            em.getTransaction().commit();
        } catch (Exception ex) {
            if (findMulta(multa.getId()) != null) {
    ...
    public void edit(Multa multa) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            Coche vehiculoOld = persistentMulta.getVehiculo();
            Coche vehiculoNew = multa.getVehiculo();
            if (vehiculoNew != null) {
                vehiculoNew = em.getReference(vehiculoNew.getClass(), vehiculoNew.getMatricula());
                multa.setVehiculo(vehiculoNew);
            }
            multa = em.merge(multa);
            if (vehiculoOld != null && !vehiculoOld.equals(vehiculoNew)) {
                vehiculoOld.getMultaList().remove(multa);
                vehiculoOld = em.merge(vehiculoOld);
            }
            if (vehiculoNew != null && !vehiculoNew.equals(vehiculoOld)) {
                vehiculoNew.getMultaList().add(multa);
    ...
    public void destroy(String id) throws NonexistentEntityException {
        ...
        try {
            ...
            Coche vehiculo = multa.getVehiculo();
            if (vehiculo != null) {
                vehiculo.getMultaList().remove(multa);
                vehiculo = em.merge(vehiculo);
            }
            em.remove(multa);
            em.getTransaction().commit();
    ...
    public Multa findMulta(String id) {
        EntityManager em = getEntityManager();
        try {
